...
import { COURSES, COURSES_DATA } from "../data/course.js";
...
const renderCourseDetail = (courseId) => {
  ...
  // ✅ Prefer DB-loaded COURSES first, fallback to static COURSES_DATA
  const course =
    (COURSES && COURSES[courseId]) ||
    (Array.isArray(COURSES_DATA) ? COURSES_DATA.find((c) => c.id === courseId) : null);
...
<div class="min-h-screen flex items-center justify-center p-8">
  ...
  // ✅ DB counts (preferred) → fallback lengths → 0
  const vCount = Number(course?.contentCounts?.videos ?? course?.content?.videos?.length ?? 0);
  const eCount = Number(course?.contentCounts?.ebooks ?? course?.content?.ebooks?.length ?? 0);
  const wCount = Number(course?.contentCounts?.workbooks ?? course?.content?.workbooks?.length ?? 0);
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 w-full">
      <div class="max-w-3xl">
        <nav class="flex items-center space-x-2 text-yellow-500 text-sm font-black uppercase tracking-widest mb-6">
          ...
          <span>/</span>
          <a href="#/${String(course.level || "").toLowerCase()}" class="hover:text-white transition-colors">${escapeHtml(course.level || "")} Path</a>
        </nav>
        ...
        <div class="flex flex-wrap items-center gap-6 text-slate-300">
          <div class="flex items-center space-x-2">
            ...
            <span class="font-bold">${escapeHtml(course.duration || "Self-paced")}</span>
          </div>
          <div class="flex items-center space-x-2">
            ...
            <span class="font-bold">Led by ${escapeHtml(course.instructor || "Cikgu Kripto")}</span>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-16">
          <div>
            ...
            <div class="prose prose-slate prose-lg max-w-none">
              <div class="text-xl text-slate-600 leading-relaxed mb-6">
                ${formatTextWithBreaks(course.description || "")}
              </div>

              <p class="text-slate-600 leading-relaxed">
                This curriculum is built from years of real market experience, designed to help you bridge the gap between “knowing” crypto and actually executing with a clear plan in live markets.
              </p>
              <p class="text-slate-600 leading-relaxed">
                Throughout this course, you’ll learn not only the step-by-step process (CEX to DEX), but also the “why” behind market cycles, narratives, and risk management that separates random gamblers from disciplined investors. We focus on consistency, position sizing, and decision-making under pressure.
              </p>
            </div>
          </div>
                ...
                <div class="group flex items-start space-x-6 p-8 bg-slate-50 rounded-3xl border border-slate-100 transition-all hover:bg-white hover:shadow-xl hover:shadow-yellow-50 hover:-translate-y-1">
                  ...
                  <div>
                    ...
                    <p class="text-slate-500 leading-relaxed">
                      Hands-on breakdown with real examples, checklists, and common mistakes people buat masa masuk market.
                    </p>
                  </div>
                </div>`
                )
            ...
            <div class="relative z-10">
              ...
              <div class="grid md:grid-cols-3 gap-8">
                <div class="p-6 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
                  ...
                  <p class="text-sm text-slate-400">Over ${vCount} high-definition video modules with structured walkthroughs.</p>
                </div>
                ...
                <div class="p-6 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
                  ...
                  <p class="text-sm text-slate-400">Lifetime access to ${eCount} crypto guides—fundamentals, risk, portfolio, bull run.</p>
                </div>
                ...
                <div class="p-6 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
                  ...
                  <p class="text-sm text-slate-400">Practice with ${wCount} hands-on Google Sheet workbooks (risk plan, journal, portfolio).</p>
                </div>
              </div>
            </div>
          ...
          <div class="sticky top-28 space-y-8">
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/60 p-10 overflow-hidden relative">
              <div class="relative z-10">
                ...
                <div class="text-5xl font-black text-slate-900 mb-2">RM${escapeHtml(String(course.price ?? ""))}</div>
                ...
                <div class="space-y-4">
                  <div class="bg-emerald-50 border border-emerald-100 p-6 rounded-2xl text-center">
                    ...
                    <p class="text-xs text-emerald-600">ID: ${escapeHtml(String(course.id || "").toUpperCase())}-ACCESS</p>
                  </div>
  ...
  <section class="py-24 bg-slate-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="bg-white rounded-[3rem] p-12 md:p-20 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-center gap-16">
        <div class="md:w-1/3">
          <img src="https://ui-avatars.com/api/?name=${encodeQS(course.instructor || "Cikgu Kripto")}&size=512&background=eab308&color=fff" class="w-full aspect-square object-cover rounded-[2.5rem] shadow-2xl shadow-yellow-100" alt="${escapeHtml(course.instructor || "Cikgu Kripto")}" />
        </div>
        <div class="md:w-2/3">
          <span class="text-yellow-500 font-black uppercase tracking-widest text-sm mb-4 block">Meet Your Instructor</span>
          <h2 class="text-4xl md:text-5xl font-black text-slate-900 mb-6">${escapeHtml(course.instructor || "Cikgu Kripto")}</h2>
          <p class="text-xl text-slate-500 leading-relaxed mb-8 italic">
            “My mission is to transform students into confident crypto investors by teaching not just the ‘how’, but the ‘why’ behind market cycles, risk management, and smart decision-making.”
          </p>
          <div class="grid grid-cols-2 gap-8">
            <div>
              <div class="text-2xl font-black text-slate-900">12+ Years</div>
              <div class="text-slate-400 text-sm font-bold">Experience</div>
            </div>
            <div>
              <div class="text-2xl font-black text-slate-900">5k+</div>
              <div class="text-slate-400 text-sm font-bold">Students</div>
            </div>
          </div>
        </div>
      </div>
  ...
  <section class="py-20 border-t border-slate-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex flex-wrap justify-center items-center gap-12 grayscale opacity-40">
        <div class="text-2xl font-black text-slate-900">CRYPTOQUANT</div>
        <div class="text-2xl font-black text-slate-900">GLASSNODE</div>
        <div class="text-2xl font-black text-slate-900">SANTIMENT</div>
        <div class="text-2xl font-black text-slate-900">COINMARKETCAP</div>
        <div class="text-2xl font-black text-slate-900">TRADINGVIEW</div>
      </div>
    </div>
  </section>
</div>
